...
namespace CCPad.Localization
{
    ...
    public static class Loc
    {
        ...
        private static readonly Dictionary<string, string[]> _t = new()
        {
            ...
            ["mem_warn_launch_v2"] = new[] { "当前已有 {0} 个 CC Pad 窗口。物理内存已用 {1}%（剩余 {2} GB），系统提交内存已用 {3}%（剩余 {4} GB）。继续多开可能导致 WebView2 或所有窗口异常；建议先冻结空闲标签。", "{0} CC Pad windows are running. Physical memory is at {1}% ({2} GB free), and system commit is at {3}% ({4} GB free). Opening more may destabilize WebView2 or every window; freeze idle tabs first.", "目前已有 {0} 個 CC Pad 視窗。實體記憶體已用 {1}%（剩餘 {2} GB），系統認可記憶體已用 {3}%（剩餘 {4} GB）。繼續多開可能導致 WebView2 或所有視窗異常；建議先凍結閒置分頁。", "{0} CC Pad-Fenster werden ausgeführt. Der physische Speicher ist zu {1} % belegt ({2} GB frei), der Commit-Speicher zu {3} % ({4} GB frei). Weitere Fenster können WebView2 oder alle Fenster destabilisieren; frieren Sie zuerst inaktive Tabs ein.", "CC Pad ウィンドウが {0} 個実行中です。物理メモリは {1}% 使用（空き {2} GB）、システム コミットは {3}% 使用（空き {4} GB）です。さらに開くと WebView2 やすべてのウィンドウが不安定になる可能性があります。先に非アクティブなタブを凍結してください。", "{0} fenêtres CC Pad sont ouvertes. La mémoire physique est utilisée à {1} % ({2} Go libres) et l'engagement système à {3} % ({4} Go libres). En ouvrir davantage peut déstabiliser WebView2 ou toutes les fenêtres ; gelez d'abord les onglets inactifs.", "CC Pad 창 {0}개가 실행 중입니다. 실제 메모리는 {1}% 사용 중이며({2} GB 남음), 시스템 커밋은 {3}% 사용 중입니다({4} GB 남음). 창을 더 열면 WebView2 또는 모든 창이 불안정해질 수 있으니 먼저 유휴 탭을 동결하세요.", "Hay {0} ventanas de CC Pad en ejecución. La memoria física está al {1}% ({2} GB libres) y la memoria comprometida del sistema al {3}% ({4} GB libres). Abrir más puede desestabilizar WebView2 o todas las ventanas; congela primero las pestañas inactivas.", "Sono in esecuzione {0} finestre di CC Pad. La memoria fisica è al {1}% ({2} GB liberi) e la memoria impegnata di sistema al {3}% ({4} GB liberi). Aprirne altre può rendere instabile WebView2 o tutte le finestre; congela prima le schede inattive." },
            ["mem_warn_runtime_v2"] = new[] { "资源压力过高：物理内存已用 {0}%（剩余 {1} GB），系统提交内存已用 {2}%（剩余 {3} GB）。建议立即冻结空闲标签或关闭部分窗口；CC Pad 不会自动结束正在工作的会话。", "Resource pressure is high: physical memory is at {0}% ({1} GB free), and system commit is at {2}% ({3} GB free). Freeze idle tabs or close some windows now; CC Pad will not automatically stop working sessions.", "資源壓力過高：實體記憶體已用 {0}%（剩餘 {1} GB），系統認可記憶體已用 {2}%（剩餘 {3} GB）。建議立即凍結閒置分頁或關閉部分視窗；CC Pad 不會自動結束正在工作的工作階段。", "Hoher Ressourcendruck: Der physische Speicher ist zu {0} % belegt ({1} GB frei), der Commit-Speicher zu {2} % ({3} GB frei). Frieren Sie jetzt inaktive Tabs ein oder schließen Sie Fenster; CC Pad beendet keine aktiven Sitzungen automatisch.", "リソース負荷が高くなっています。物理メモリは {0}% 使用（空き {1} GB）、システム コミットは {2}% 使用（空き {3} GB）です。非アクティブなタブを凍結するか、ウィンドウを閉じてください。CC Pad が作業中のセッションを自動終了することはありません。", "La pression sur les ressources est élevée : mémoire physique à {0} % ({1} Go libres), engagement système à {2} % ({3} Go libres). Gelez les onglets inactifs ou fermez des fenêtres ; CC Pad n'arrête pas automatiquement les sessions actives.", "리소스 사용량이 높습니다. 실제 메모리는 {0}% 사용 중이며({1} GB 남음), 시스템 커밋은 {2}% 사용 중입니다({3} GB 남음). 유휴 탭을 동결하거나 일부 창을 닫으세요. CC Pad는 작업 중인 세션을 자동 종료하지 않습니다.", "La presión de recursos es alta: memoria física al {0}% ({1} GB libres) y memoria comprometida del sistema al {2}% ({3} GB libres). Congela pestañas inactivas o cierra ventanas; CC Pad no detendrá automáticamente las sesiones activas.", "La pressione sulle risorse è alta: memoria fisica al {0}% ({1} GB liberi) e memoria impegnata di sistema al {2}% ({3} GB liberi). Congela le schede inattive o chiudi alcune finestre; CC Pad non interrompe automaticamente le sessioni attive." },
            ...
            ["template_save"] = new[] { "保存为冻结模板...", "Save as frozen template…", "儲存為凍結範本…", "Als eingefrorene Vorlage speichern…", "凍結テンプレートとして保存…", "Enregistrer comme modèle figé…", "고정 템플릿으로 저장…", "Guardar como plantilla congelada…", "Salva come modello congelato…" },
            ["template_open_new"] = new[] { "从冻结模板新开窗口...", "New window from frozen template…", "從凍結範本開新視窗…", "Neues Fenster aus eingefrorener Vorlage…", "凍結テンプレートから新しいウィンドウ…", "Nouvelle fenêtre depuis un modèle figé…", "고정 템플릿에서 새 창…", "Nueva ventana desde plantilla congelada…", "Nuova finestra da modello congelato…" },
            ["template_filetype"] = new[] { "CCPad 冻结模板", "CCPad frozen template", "CCPad 凍結範本", "Eingefrorene CCPad-Vorlage", "CCPad 凍結テンプレート", "Modèle figé CCPad", "CCPad 고정 템플릿", "Plantilla congelada de CCPad", "Modello congelato CCPad" },
            ["template_save_new"] = new[] { "保存当前窗口为新冻结模板", "Save current window as new frozen template", "將目前視窗儲存為新凍結範本", "Aktuelles Fenster als neue eingefrorene Vorlage speichern", "現在のウィンドウを新しい凍結テンプレートとして保存", "Enregistrer la fenêtre comme nouveau modèle figé", "현재 창을 새 고정 템플릿으로 저장", "Guardar ventana como nueva plantilla congelada", "Salva la finestra come nuovo modello congelato" },
            ["template_library"] = new[] { "冻结模板", "Frozen templates", "凍結範本", "Eingefrorene Vorlagen", "凍結テンプレート", "Modèles figés", "고정 템플릿", "Plantillas congeladas", "Modelli congelati" },
            ["template_default_prefix"] = new[] { "冻结模板", "Frozen template", "凍結範本", "Eingefrorene Vorlage", "凍結テンプレート", "Modèle figé", "고정 템플릿", "Plantilla congelada", "Modello congelato" },
            ["template_none"] = new[] { "暂无冻结模板", "No frozen templates", "暫無凍結範本", "Keine eingefrorenen Vorlagen", "凍結テンプレートはありません", "Aucun modèle figé", "고정 템플릿 없음", "No hay plantillas congeladas", "Nessun modello congelato" },
            ["template_restore_current"] = new[] { "恢复到当前窗口", "Restore into current window", "還原到目前視窗", "Im aktuellen Fenster wiederherstellen", "現在のウィンドウに復元", "Restaurer dans la fenêtre actuelle", "현재 창에 복원", "Restaurar en la ventana actual", "Ripristina nella finestra corrente" },
            ["template_overwrite"] = new[] { "用当前窗口覆盖", "Overwrite with current window", "以目前視窗覆寫", "Mit aktuellem Fenster überschreiben", "現在のウィンドウで上書き", "Remplacer par la fenêtre actuelle", "현재 창으로 덮어쓰기", "Sobrescribir con la ventana actual", "Sovrascrivi con la finestra corrente" },
            ["template_rename"] = new[] { "重命名", "Rename", "重新命名", "Umbenennen", "名前を変更", "Renommer", "이름 바꾸기", "Cambiar nombre", "Rinomina" },
            ["template_export"] = new[] { "导出配置文件...", "Export configuration…", "匯出設定檔…", "Konfigurationsdatei exportieren…", "設定ファイルをエクスポート…", "Exporter la configuration…", "구성 파일 내보내기…", "Exportar configuración…", "Esporta configurazione…" },
            ["template_import"] = new[] { "导入冻结模板配置...", "Import frozen-template configuration…", "匯入凍結範本設定…", "Vorlagenkonfiguration importieren…", "凍結テンプレート設定をインポート…", "Importer une configuration de modèle…", "고정 템플릿 구성 가져오기…", "Importar configuración de plantilla…", "Importa configurazione modello…" },
            ["template_delete"] = new[] { "删除", "Delete", "刪除", "Löschen", "削除", "Supprimer", "삭제", "Eliminar", "Elimina" },
            ["template_restore_title"] = new[] { "恢复冻结模板？", "Restore frozen template?", "還原凍結範本？", "Eingefrorene Vorlage wiederherstellen?", "凍結テンプレートを復元しますか？", "Restaurer le modèle figé ?", "고정 템플릿을 복원할까요?", "¿Restaurar plantilla congelada?", "Ripristinare il modello congelato?" },
            ["template_restore_body"] = new[] { "当前窗口会被模板布局替换；现有布局会先保存到已关闭会话历史中。", "The template replaces the current window; the existing layout is saved to closed-session history first.", "範本會取代目前視窗；現有版面會先儲存到已關閉工作階段記錄。", "Die Vorlage ersetzt das aktuelle Fenster; das vorhandene Layout wird zuvor im Sitzungsverlauf gespeichert.", "テンプレートで現在のウィンドウを置き換えます。現在のレイアウトは履歴に保存されます。", "Le modèle remplace la fenêtre actuelle ; la disposition existante est d'abord enregistrée dans l'historique.", "템플릿이 현재 창을 대체하며 기존 레이아웃은 먼저 기록에 저장됩니다.", "La plantilla reemplaza la ventana actual; el diseño existente se guarda primero en el historial.", "Il modello sostituisce la finestra corrente; il layout esistente viene prima salvato nella cronologia." },
            ["template_overwrite_title"] = new[] { "覆盖冻结模板？", "Overwrite frozen template?", "覆寫凍結範本？", "Eingefrorene Vorlage überschreiben?", "凍結テンプレートを上書きしますか？", "Remplacer le modèle figé ?", "고정 템플릿을 덮어쓸까요?", "¿Sobrescribir plantilla congelada?", "Sovrascrivere il modello congelato?" },
            ["template_overwrite_body"] = new[] { "将“{0}”替换为当前窗口的布局和会话。", "Replace “{0}” with the current window layout and sessions.", "以目前視窗的版面與工作階段取代「{0}」。", "„{0}“ durch Layout und Sitzungen des aktuellen Fensters ersetzen.", "「{0}」を現在のレイアウトとセッションで置き換えます。", "Remplacer « {0} » par la disposition et les sessions actuelles.", "“{0}”을 현재 창 레이아웃과 세션으로 바꿉니다.", "Reemplazar “{0}” con el diseño y las sesiones actuales.", "Sostituire “{0}” con layout e sessioni correnti." },
            ["template_delete_title"] = new[] { "删除冻结模板？", "Delete frozen template?", "刪除凍結範本？", "Eingefrorene Vorlage löschen?", "凍結テンプレートを削除しますか？", "Supprimer le modèle figé ?", "고정 템플릿을 삭제할까요?", "¿Eliminar plantilla congelada?", "Eliminare il modello congelato?" },
            ["template_delete_body"] = new[] { "将永久删除“{0}”。已导出的配置文件不受影响。", "Permanently delete “{0}”. Exported configuration files are unaffected.", "將永久刪除「{0}」。已匯出的設定檔不受影響。", "„{0}“ dauerhaft löschen. Exportierte Dateien bleiben erhalten.", "「{0}」を完全に削除します。エクスポート済みファイルには影響しません。", "Supprimer définitivement « {0} ». Les fichiers exportés ne sont pas affectés.", "“{0}”을 영구 삭제합니다. 내보낸 파일은 영향을 받지 않습니다.", "Eliminar permanentemente “{0}”. Los archivos exportados no se verán afectados.", "Eliminare definitivamente “{0}”. I file esportati non saranno modificati." },
            ...
            ["proj_open_codex_remote"] = new[] { "用 Codex@167 打开", "Open with Codex@167", "用 Codex@167 開啟", "Mit Codex@167 öffnen", "Codex@167 で開く", "Ouvrir avec Codex@167", "Codex@167로 열기", "Abrir con Codex@167", "Apri con Codex@167" },
            ...
            ["btn_lastcmd"] = new[] { "上条", "Last", "上條", "Zuletzt", "直前", "Dernière", "최근", "Última", "Ultimo" },
            ["tip_lastcmd"] = new[] { "上一条命令信息栏：每个终端顶部显示最近发出的命令（Alt+L 开/关）", "Last-command bar: shows the most recent command at the top of each terminal (Alt+L to toggle)", "上一條命令資訊列：每個終端機頂部顯示最近送出的命令（Alt+L 開/關）", "Leiste „Letzter Befehl“: zeigt oben in jedem Terminal den zuletzt gesendeten Befehl (Alt+L)", "直前のコマンド バー: 各ターミナル上部に最後に送信したコマンドを表示 (Alt+L)", "Barre de dernière commande : affiche la commande la plus récente en haut de chaque terminal (Alt+L)", "최근 명령 표시줄: 각 터미널 상단에 마지막으로 보낸 명령을 표시 (Alt+L)", "Barra de último comando: muestra el comando más reciente en la parte superior de cada terminal (Alt+L)", "Barra ultimo comando: mostra il comando più recente in cima a ogni terminale (Alt+L)" },
            ["btn_files"] = new[] { "文件", "Files", "檔案", "Dateien", "ファイル", "Fichiers", "파일", "Archivos", "File" },
            ["tip_files"] = new[] { "文件管理器：浏览当前项目目录", "File manager: browse the current project folder", "檔案管理員：瀏覽目前專案目錄", "Dateimanager: aktuellen Projektordner durchsuchen", "ファイル マネージャー: 現在のプロジェクト フォルダーを参照", "Gestionnaire de fichiers : parcourir le dossier du projet actuel", "파일 관리자: 현재 프로젝트 폴더 탐색", "Administrador de archivos: examinar la carpeta del proyecto actual", "Gestione file: sfoglia la cartella del progetto corrente" },
            ["btn_auto"] = new[] { "回车", "Enter", "回車", "Enter", "確認", "Entrée", "엔터", "Intro", "Invio" },
            ["tip_auto"] = new[] { "自动回车：AI 弹出确认提示时自动替你按回车", "Auto-Enter: presses Enter for you when the AI asks for confirmation", "自動回車：AI 彈出確認提示時自動替你按 Enter", "Auto-Enter: drückt automatisch die Eingabetaste, wenn die KI um Bestätigung bittet", "自動 Enter: AI が確認を求めたとき自動で Enter を押します", "Entrée auto : appuie sur Entrée à votre place quand l'IA demande confirmation", "자동 엔터: AI가 확인을 요청하면 자동으로 Enter를 누릅니다", "Intro automático: pulsa Intro por ti cuando la IA pide confirmación", "Invio automatico: preme Invio al posto tuo quando l'IA chiede conferma" },
            ["btn_stage"] = new[] { "寄存", "Stage", "寄存", "Puffer", "待機", "File", "대기열", "Cola", "Coda" },
            ["tip_stage"] = new[] { "命令寄存模式：输入先排队，会话空闲时自动逐条发送", "Command staging: input is queued first and sent one line at a time when the session is idle", "命令寄存模式：輸入先排隊，工作階段空閒時自動逐條送出", "Befehls-Warteschlange: Eingaben werden zuerst gesammelt und bei Leerlauf nacheinander gesendet", "コマンド待機モード: 入力をキューに入れ、セッションがアイドルのとき 1 件ずつ自動送信", "Mise en attente des commandes : la saisie est mise en file et envoyée ligne par ligne quand la session est inactive", "명령 대기 모드: 입력을 먼저 대기열에 넣고 세션이 유휴 상태일 때 한 줄씩 자동 전송", "Cola de comandos: la entrada se pone en cola y se envía línea a línea cuando la sesión está inactiva", "Coda comandi: l'input viene messo in coda e inviato una riga alla volta quando la sessione è inattiva" },
            ["btn_about"] = new[] { "关于", "About", "關於", "Info", "情報", "À propos", "정보", "Acerca", "Info" },

            // ── File manager panel ──
            ["fm_up"] = new[] { "返回上一层", "Up one level", "返回上一層", "Eine Ebene nach oben", "1 つ上へ", "Remonter d'un niveau", "상위 폴더로", "Subir un nivel", "Su di un livello" },
            ["fm_refresh"] = new[] { "刷新", "Refresh", "重新整理", "Aktualisieren", "更新", "Actualiser", "새로 고침", "Actualizar", "Aggiorna" },
            ["fm_path_tip"] = new[] { "当前路径，回车跳转", "Current path — press Enter to go", "目前路徑，按 Enter 跳轉", "Aktueller Pfad – Enter zum Öffnen", "現在のパス — Enter で移動", "Chemin actuel — Entrée pour y aller", "현재 경로 — Enter로 이동", "Ruta actual: Intro para ir", "Percorso corrente — Invio per aprire" },
            ["fm_search_ph"] = new[] { "检索当前目录…", "Search this folder…", "搜尋目前目錄…", "Diesen Ordner durchsuchen…", "このフォルダーを検索…", "Rechercher dans ce dossier…", "이 폴더에서 검색…", "Buscar en esta carpeta…", "Cerca in questa cartella…" },
            ["fm_open"] = new[] { "打开", "Open", "開啟", "Öffnen", "開く", "Ouvrir", "열기", "Abrir", "Apri" },
            ["fm_copy_path"] = new[] { "复制文件路径", "Copy file path", "複製檔案路徑", "Dateipfad kopieren", "ファイル パスをコピー", "Copier le chemin du fichier", "파일 경로 복사", "Copiar ruta del archivo", "Copia percorso file" },
            ["fm_delete_title"] = new[] { "删除到回收站", "Delete to Recycle Bin", "刪除到資源回收筒", "In den Papierkorb verschieben", "ごみ箱へ削除", "Supprimer vers la corbeille", "휴지통으로 삭제", "Eliminar a la papelera", "Elimina nel cestino" },
            ["fm_delete_body"] = new[] { "将 “{0}” 移到回收站？", "Move “{0}” to the Recycle Bin?", "將「{0}」移到資源回收筒？", "„{0}“ in den Papierkorb verschieben?", "「{0}」をごみ箱に移動しますか？", "Déplacer « {0} » vers la corbeille ?", "“{0}”을(를) 휴지통으로 이동할까요?", "¿Mover “{0}” a la papelera?", "Spostare “{0}” nel cestino?" },
            ["fm_open_failed"] = new[] { "打开失败", "Couldn't open", "開啟失敗", "Öffnen fehlgeschlagen", "開けませんでした", "Échec de l'ouverture", "열기 실패", "Error al abrir", "Apertura non riuscita" },
            ["fm_delete_failed"] = new[] { "删除失败", "Couldn't delete", "刪除失敗", "Löschen fehlgeschlagen", "削除できませんでした", "Échec de la suppression", "삭제 실패", "Error al eliminar", "Eliminazione non riuscita" },
            ["fm_rename_failed"] = new[] { "重命名失败", "Couldn't rename", "重新命名失敗", "Umbenennen fehlgeschlagen", "名前を変更できませんでした", "Échec du renommage", "이름 바꾸기 실패", "Error al cambiar el nombre", "Rinomina non riuscita" },

            // ── Terminal pane errors (zh-Hans / en / zh-Hant; others fall back to English) ──
            ["pane_error_title"] = new[] { "终端页面异常", "Terminal page error", "終端機頁面異常" },
            ["retry"] = new[] { "重试", "Retry", "重試", "Wiederholen", "再試行", "Réessayer", "다시 시도", "Reintentar", "Riprova" },
            ["pane_err_render"] = new[] { "渲染进程崩溃（{0}），自动恢复失败。会话进程仍在运行，点击重试重新加载页面。", "The renderer crashed ({0}) and automatic recovery failed. The session process is still running — click Retry to reload the page.", "轉譯程序當機（{0}），自動復原失敗。工作階段處理程序仍在執行，點擊重試重新載入頁面。" },
            ["pane_err_process"] = new[] { "WebView2 进程异常（{0}）。会话进程仍在运行，点击重试重新初始化。", "WebView2 process failure ({0}). The session process is still running — click Retry to reinitialize.", "WebView2 處理程序異常（{0}）。工作階段處理程序仍在執行，點擊重試重新初始化。" },
            ["pane_recovered"] = new[] { "[终端页面已自动恢复；之前的滚动内容丢失，会话仍在运行]", "[Terminal page auto-recovered; earlier scrollback was lost — the session is still running]", "[終端機頁面已自動復原；先前的捲動內容遺失，工作階段仍在執行]" },
            ["pane_retry_failed"] = new[] { "重试失败：{0}", "Retry failed: {0}", "重試失敗：{0}" },
            ["pane_init_failed"] = new[] { "终端初始化失败：{0}", "Terminal initialization failed: {0}", "終端機初始化失敗：{0}" },
            ["pane_err_loaded"] = new[] { "终端控件未进入可视树（Loaded 超时）。", "The terminal control never entered the visual tree (Loaded timed out).", "終端機控制項未進入可視樹（Loaded 逾時）。" },
            ["pane_err_wv_init"] = new[] { "WebView2 初始化超时（{0} 秒）。", "WebView2 initialization timed out ({0} s).", "WebView2 初始化逾時（{0} 秒）。" },
            ["pane_err_ready"] = new[] { "终端页面在 {0} 秒内未就绪。", "The terminal page was not ready within {0} seconds.", "終端機頁面在 {0} 秒內未就緒。" },
            ["pane_err_cold"] = new[] { "终端页面未能就绪（冷启动失败）。", "The terminal page never became ready (cold start failed).", "終端機頁面未能就緒（冷啟動失敗）。" },
            ["clip_no_image"] = new[] { "剪贴板里没有图片", "No image on the clipboard", "剪貼簿裡沒有圖片" },
            ["clip_read_failed"] = new[] { "读取剪贴板图片失败", "Couldn't read the image from the clipboard", "讀取剪貼簿圖片失敗" },

            // ── In-page terminal UI (swapped into TerminalHtml at load; values used
            //    inside JS single-quoted literals must not contain ' ) ──
            ["lastcmd_lbl"] = new[] { "▸ 上一条", "▸ Last", "▸ 上一條" },
            ["lastcmd_title"] = new[] { "点击复制全文 · Alt+L 关闭", "Click to copy · Alt+L to hide", "點擊複製全文 · Alt+L 關閉" },
            ["lastcmd_copied"] = new[] { "已复制 ✓", "Copied ✓", "已複製 ✓" },
            ["stage_status_waiting"] = new[] { "等待中 · 进入等待自动发送", "Waiting · auto-sends when idle", "等待中 · 進入等待自動傳送" },
            ["stage_status_working"] = new[] { "工作中 · 等它空闲", "Working · waiting for idle", "工作中 · 等待空閒" },
            ["stage_status_disc"] = new[] { "已断开 · 暂停发送", "Disconnected · sending paused", "已斷開 · 暫停傳送" },
            ["stage_bulk_toggle"] = new[] { "＋ 批量导入", "＋ Bulk import", "＋ 批次匯入" },
            ["stage_queued"] = new[] { "条待发", "queued", "條待發" },
            ["stage_empty"] = new[] { "队列为空 —— 在下面输入,回车逐条寄存", "Queue is empty — type below, press Enter to stage each line", "佇列為空 —— 在下面輸入，Enter 逐條寄存" },
            ["stage_input_ph"] = new[] { "输入命令,回车寄存(Shift+Enter 换行)", "Type a command — Enter stages it (Shift+Enter for a new line)", "輸入命令，Enter 寄存（Shift+Enter 換行）" },
            ["stage_bulk_ph"] = new[] { "批量导入:每行一条命令,空行忽略。粘贴一整段后点导入。", "Bulk import: one command per line, blank lines ignored. Paste a block, then click Import.", "批次匯入：每行一條命令，空行忽略。貼上整段後點匯入。" },
            ["stage_bulk_import"] = new[] { "导入队列", "Import to queue", "匯入佇列" },
            ["stage_hint"] = new[] { "寄存模式:输入排进队列,会话空闲时自动逐条发出。点✎或双击可编辑(编辑中暂停发送),Alt+V 贴图,Alt+` 退出寄存。", "Staging mode: input is queued and sent one line at a time when the session goes idle. Click ✎ or double-click to edit (sending pauses while editing). Alt+V pastes an image, Alt+` leaves staging.", "寄存模式：輸入排入佇列，工作階段空閒時自動逐條送出。點 ✎ 或按兩下可編輯（編輯中暫停傳送），Alt+V 貼圖，Alt+` 離開寄存。" },
            ["js_edit_tip"] = new[] { "编辑这条(双击文字也可)", "Edit this entry (double-click also works)", "編輯這條（按兩下文字也可）" },
            ["js_del_tip"] = new[] { "删除这条", "Delete this entry", "刪除這條" },
            ["js_no_image"] = new[] { "没有图片", "No image", "沒有圖片" },
            ...
            ["shell_template_type"] = new[] { "CCPad 冻结模板", "CCPad frozen template", "CCPad 凍結範本", "Eingefrorene CCPad-Vorlage", "CCPad 凍結テンプレート", "Modèle figé CCPad", "CCPad 고정 템플릿", "Plantilla congelada de CCPad", "Modello congelato CCPad" },
